...
peak_widths = []
for c in channel:
    p0 = [energy[c], 10, 10] # initial guesses for parameters
    popt, pcov = curve_fit(lorentzian, energy_smooth[c-10:c+10], energy[c-10:c+10], p0=p0)
    peak_intensities.append(popt[0])
    peak_widths.append(popt[2])

# Plot spectrum with identified peaks and Lorentzian fits
plt.plot(energy, label='Spectrum')
for c, e, i, w in zip(channel, peak_energies, peak_intensities, peak_widths):
    plt.plot(energy_smooth[c-10:c+10], lorentzian(energy_smooth[c-10:c+10], i, e, w), label='Peak at %.1f keV' % e)
